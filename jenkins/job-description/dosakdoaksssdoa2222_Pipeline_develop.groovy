...
def job_config = [
        job: [
                name: "dosakdoaksssdoa2222_Pipeline_develop",
                agent: "maven"
        ],
        git: [
                branch: "develop"
...
def customLib = library identifier: 'dosakdoaksssdoa2222_JSL@develop', retriever: modernSCM(
  [\$class: 'GitSCMSource',
   remote: 'https://github.developer.allianz.io/kkanto/dosakdoaksssdoa2222_lib.git',
   credentialsId: 'git-token-credentials']) 
...
def jslCustom     = customLib.de.allianz.dosakdoaksssdoa2222.new()
    ...
    stages {
        ...
        stage('Checkout') {
            steps {
                echo "checkout"
                script {
                   jslGit.checkout( config, "kkanto", "dosakdoaksssdoa2222", job_config.git.branch)
               }
            }    
        }
        stage('Build') {
...
def job = pipelineJob("dosakdoaksssdoa2222_Pipeline_develop");

job.with {

    authenticationToken('Z6aVBIECYuqOmaZKN3OUXEgZpAOh2qhU')
